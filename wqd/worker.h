#ifndef __WQD_WORKER_H__
#define __WQD_WORKER_H__
...
public:
    ...
    static WorkerGroup::ptr Create(uint32_t batch_size, wqd::Scheduler* s = wqd::Scheduler::GetThis()) {
        return std::make_shared<WorkerGroup>(batch_size, s);
    }

    WorkerGroup(uint32_t batch_size, wqd::Scheduler* s = wqd::Scheduler::GetThis());
    ~WorkerGroup();
...
public:
    ...
    void schedule(const std::string& name, FiberOrCb fc, int thread = -1) {
        ...
        if(s) {
            ...
            static wqd::Logger::ptr s_logger = WQD_LOG_NAME("system");
            WQD_LOG_ERROR(s_logger) << "schedule name=" << name
                << " not exists";
        }
    }
...
typedef wqd::Singleton<WorkerManager> WorkerMgr;
