...
int32_t FunctionServlet::handle(wqd::http::HttpRequest::ptr request
...
ServletDispatch::ServletDispatch()
    ...
    m_default.reset(new NotFoundServlet("wqd/1.0"));
}

int32_t ServletDispatch::handle(wqd::http::HttpRequest::ptr request
               , wqd::http::HttpResponse::ptr response
               , wqd::http::HttpSession::ptr session) {
...
int32_t NotFoundServlet::handle(wqd::http::HttpRequest::ptr request
                   , wqd::http::HttpResponse::ptr response
                   , wqd::http::HttpSession::ptr session) {
    response->setStatus(wqd::http::HttpStatus::NOT_FOUND);
    response->setHeader("Server", "wqd/1.0.0");
